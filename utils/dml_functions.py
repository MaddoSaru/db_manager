...
from utils.general_db_functions import select_database, show_tables_list, create_columns_config, table_describe_dict
from utils.general_functions import camel, select_terminal_menu_option, add_config_database, make_request
    ...
    if action_txt == 'Insert Registers':
        ...
        if insert_registers_action_txt == 'Insert Request Registers':
            ...
            for column in table_describe:
                logging.info(f"Column {camel(column)} Has Data Type {table_describe[column]}")
                value = str(input(f"Select Column {camel(column)} Value: "))
                if column != list(table_describe)[-1]:
                    columns += column + ","
                    values += '"' + value + '",' if table_describe[column] in ['datetime', 'char(200)'] else value + ","
                else:
                    columns += column
                    values += '"' + value + '"' if table_describe[column] in ['datetime', 'char(200)'] else value
            format_query_str = query_str.format(table_name = table_name, columns = columns, values = values)
